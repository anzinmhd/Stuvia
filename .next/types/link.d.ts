...
declare namespace __next_route_internal_types__ {
  ...
  type StaticRoutes = 
    ...
    | `/api/class-changes`
    | `/api/profile`
    | `/api/user-class-changes`
    | `/api/templates`
    | `/api/user-exists`
    | `/settings`
  type DynamicRoutes<T extends string = string> = 
    | `/api/auth/${CatchAllSlug<T>}`
    | `/api/subjects/${SafeSlug<T>}`
    | `/api/timetable/${SafeSlug<T>}`
